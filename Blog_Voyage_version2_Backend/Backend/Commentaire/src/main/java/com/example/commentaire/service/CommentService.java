...
public class CommentService {
    ...
    private final String USER_URL = "http://localhost:8888/SERVICE-UTILISATEUR";
    private final String POST_URL = "http://localhost:8888/SERVICE-POST";

    public List<CommentResponse> findAll() {
        List<Commentaire> comments = commentRepository.findAll();
        ResponseEntity<UserComment[]> userResponse =
                restTemplate.getForEntity(this.USER_URL+"/api/ueser", UserComment[].class);
        UserComment[] usersComment = userResponse.getBody();

        ResponseEntity<PostComment[]> postResponse =
                restTemplate.getForEntity(this.POST_URL+"/api/posts", PostComment[].class);
    ...
    public CommentResponse findById(Long id) throws Exception {
        ...
        UserComment userComment = restTemplate.getForObject(this.USER_URL+"/api/ueser/" + comment.getUser_id(), UserComment.class);
        PostComment postComment = restTemplate.getForObject(this.POST_URL+"/api/posts/" + comment.getPost_id(), PostComment.class);
    ...
    public List<CommentResponse> findCommentsByPostId(Long postId) {
        List<Commentaire> comments = commentRepository.findCommentByPostId(postId);
        ...
        return comments.stream()
                .map(comment -> mapToCommentResponse(comment, usersComment, postsComment))
                .toList();
    }
