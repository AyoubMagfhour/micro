...
public class FavoriteService {
    ...
    public List<FavoriteResponse> findFavoritesByUserId(Long userId) {
        List<Favorite> favorites = favoriteRepository.findByUserFavId(userId);

        // Fetch user and post details for each favorite
        ResponseEntity<UserFav> userResponse = restTemplate.getForEntity(USER_URL + userId, UserFav.class);
        UserFav user = userResponse.getBody();

        return favorites.stream()
                .map(favorite -> mapToFavoriteResponse(favorite, user))
                .collect(Collectors.toList());
    }

    private FavoriteResponse mapToFavoriteResponse(Favorite favorite, UserFav user) {
        ResponseEntity<PostFav> postResponse = restTemplate.getForEntity(POST_URL + favorite.getPostfav_id(), PostFav.class);
        PostFav post = postResponse.getBody();
