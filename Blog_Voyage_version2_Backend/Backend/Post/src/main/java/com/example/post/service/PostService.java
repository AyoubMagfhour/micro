...
public class PostService {
    ...
    public List<PostResponse> findPostsByUserId(Long userId) throws Exception {
    ...
    public List<PostResponse> findByPays(String pays) {
        List<Post> postsByPays = postRepository.findByPays(pays);
        ResponseEntity<Userpost []> response =
                restTemplate.getForEntity(this.URL + "/api/ueser", Userpost[].class);
        Userpost[] userspost = response.getBody();

        return postsByPays.stream().map((Post post) ->
                mapToPostResponse(post, userspost)).toList();
    }
